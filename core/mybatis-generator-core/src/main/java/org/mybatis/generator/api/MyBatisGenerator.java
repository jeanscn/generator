...
public class MyBatisGenerator {
    ...
    public void generate(ProgressCallback callback,
        ...
        for (Context context : contextsToRun) {
            //TODO其他类型与html类型
            context.generateFiles(callback, generatedJavaFiles, generatedXmlFiles, generatedHtmlFiles, generatedKotlinFiles, otherGeneratedFiles, warnings);
        }
    ...
    private void writeGeneratedXmlFile(GeneratedXmlFile gxf, ProgressCallback callback)
        ...
        try {
            ...
            callback.startTask(getString(
                    "Progress.15", targetFile.getName())); //$NON-NLS-1$
            writeFile(targetFile, source, gxf.getFileEncoding()); //$NON-NLS-1$
        } catch (ShellException e) {
            warnings.add(e.getMessage());
        }
    }

    private void writeGeneratedHtmlFile(GeneratedHtmlFile ghf, ProgressCallback callback) throws InterruptedException, IOException {
        File targetFile;
        String source;
        try {
            checkFileDir(ghf);
            ...
            if (!targetFile.exists() || ghf.isOverwriteFile()) {
                if (targetFile.exists()) {
                    if (ghf.isMergeable()) {
                        source = HtmlFileMergerJaxp.getMergedSource(ghf, targetFile);
                    } else if (shellCallback.isOverwriteEnabled()) {
                        source = ghf.getFormattedContent();
                        warnings.add(getString("Warning.11", targetFile.getAbsolutePath()));
                    } else {
                        source = ghf.getFormattedContent();
                        targetFile = getUniqueFileName(directory, ghf.getFileName());

...
public class SelectByColumnGenerator extends AbstractControllerElementGenerator {
     ...
     * 构造方法
     */
    public SelectByColumnGenerator(SelectByColumnGeneratorConfiguration config) {
        super();
        this.config = config;
    }
    ...
    public void addElements(TopLevelClass parentElement) {
        if (!config.getGenControllerMethod() || config.getColumns().isEmpty()) {
            return;
        }
        ...
        Method method = createMethod(config.getMethodName());
        ...
        method.addBodyLine("List<{0}> {1}s = {2}.{3}({4});"
        ...
        method.addBodyLine("if (!{0}s.isEmpty()) '{'", entityType.getShortNameFirstLowCase());
        if (introspectedTable.getRules().isGenerateVoModel()) {
            method.addBodyLine("List<{0}> {1}s = mappings.to{0}s({2}s);"
                    ,entityVoType.getShortName()
                    ,entityVoType.getShortNameFirstLowCase()
                    ,entityType.getShortNameFirstLowCase()
            );
            method.addBodyLine("return ResponseResult.success({0}s);", entityVoType.getShortNameFirstLowCase());
        }else{
            method.addBodyLine("return ResponseResult.success({0}s);", entityType.getShortNameFirstLowCase());
        }
        method.addBodyLine("} else {");
        method.addBodyLine("return ResponseResult.failure(ApiCodeEnum.NOT_FOUND, \"未找到对应的记录\");");
        method.addBodyLine("}");
